...
public:
    ...
    void SetSpeed(double value) {
        switch (direction_) {
            case Direction::NORTH: speed_ = {0.0, -value}; break;
            case Direction::SOUTH: speed_ = {0.0, value}; break;
            case Direction::WEST:  speed_ = {-value, 0.0}; break;
            case Direction::EAST:  speed_ = {value, 0.0}; break;
        }
    }
...
inline void Dog::UpdatePosition(int ms, GameSession* session) {
    ...
    if (speed_.x == 0 && speed_.y == 0) return;
    ...
    if (new_pos != attempted) speed_ = {0.0, 0.0};
    position_ = new_pos;
