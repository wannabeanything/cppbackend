...
namespace http_handler
{
    ...
    public:
        ...
        http::response<http::string_body> HandleRequest(const http::request<Body, http::basic_fields<Allocator>> &req) const
        {
            using namespace std::literals;

            if (req.method() != http::verb::get)
            {
                return MakeError(http::status::bad_request, "badRequest", "Only GET method supported", req);
            }
    ...
    private:
        ...
        template <typename Req>
        http::response<http::string_body> HandleJoinPlayer(const Req &req)const
        {
            using namespace std::literals;
            ...
            Player &player = players_.AddPlayer(session.get(), dog);

            // сохраняем сессию, если нужно
            // game_sessions_.emplace_back(std::move(session));

            json::object res_obj;
            res_obj["authToken"] = *player.GetToken(); // Token — Tagged<std::string>
        ...
        http::response<http::string_body> HandlePlayersList(const Req &req)const
        {
            ...
            {
                return MakeError(http::status::unauthorized, "invalidToken", "Authorization header is missing", req);
            }
            ...
            for (int id = 0; id < players_.GetPlayerCount(); ++id)
            {
                Player *p = players_.FindByDogIdAndMapId(id, map_id);
                if (p)
                {
    ...
    public:
        RequestHandler(model::Game &game, fs::path static_root)
            : game_{game}, static_root_{std::move(static_root)}, api_handler_(game) {}
        ...
        void operator()(http::request<Body, http::basic_fields<Allocator>> &&req, Send &&send)
        {
            ...
            {
                return send(api_handler_.HandleRequest(req));
            }
