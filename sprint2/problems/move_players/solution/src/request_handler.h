...
namespace http_handler
{
    ...
    public:
        ...
        void HandleRequest(const http::request<Body, http::basic_fields<Allocator>> &req, Send &&send)
        {
            ...
            if (target.starts_with("/api/v1/game/action"))
            {
                boost::asio::dispatch(strand_, [this, req, send = std::forward<Send>(send)]() mutable
    ...
    private:
        ...
        json::array SerializeRoads(const std::vector<model::Road> &roads) const
        {
            json::array result;
        ...
        http::response<http::string_body> HandleGameActions(const Req &req)
        {
            ...
            const auto &move = json_body.at("move").as_object();

            // 4. Валидация параметров
            if (!move.contains("x") || !move.contains("y") || !move.contains("direction") ||
                !move.at("x").is_number() || !move.at("y").is_number() || !move.at("direction").is_string())
            {
                return MakeError(http::status::bad_request, "invalidArgument", "Invalid move parameters", req);
            }

            // 5. Получение игрока по токену
            http::response<http::string_body> err;
            auto player_opt = TryExtractPlayer(req, err);
            if (!player_opt)
                return err;
            ...
            // 6. Обновляем состояние собаки
            Dog *dog = player->GetDog();
            dog->SetSpeed(move.at("x").as_double());

            const std::string dir = std::string(move.at("direction").as_string());
            if (dir == "U")
